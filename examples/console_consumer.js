...
if (
  ...
  console.log(
    'Consumes and prints values of messages from a Kafka topic via the REST proxy API wrapper.'
  );
  console.log();
  console.log(
    'Usage: node console_consumer.js [--url <api-base-url>] --topic <topic> [--group <consumer-group-name>] [--message-limit <num_messages>] [--from-beginning] [--format <avro|binary>]'
  );
...
kafka
  ...
  .join(consumerConfig, function(err, consumer_instance) {
    if (err)
      return console.log('Failed to create instance in consumer group: ' + err);

    console.log(
      'Consumer instance initialized: ' + consumer_instance.toString()
    );
...
function logShutdown(err) {
  if (err) console.log('Error while shutting down: ' + err);
  else console.log('Shutdown cleanly.');
}
