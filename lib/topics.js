...
Topics.prototype.list = function(res) {
  this.client.request(
    '/topics',
    function(err, topics_response) {
      if (err) return res(err);
      var topics = [];
      for (var i = 0; i < topics_response.length; i++) {
        topics.push(
          new Topic(this.client, topics_response[i], topics_response[i])
        );
      }
      res(null, topics);
    }.bind(this)
...
Topic.prototype.produce = function() {
  ...
  if (schemas[0] != null) {
    ...
    else request['key_schema'] = schemas[0].toSchemaString();
  }
  if (schemas[1] != null) {
    if (schemas[1].toSchemaID())
      request['value_schema_id'] = schemas[1].toSchemaID();
    else request['value_schema'] = schemas[1].toSchemaString();
  }
  this.client.post(
    this.getPath(),
    request,
    schemas[0] || schemas[1],
    function(err, produce_response) {
      if (err) {
        if (res) res(err);
      } else {
...
Topic.prototype.__defineGetter__('partitions', function() {
  if (!this._partitions) this._partitions = new Partitions(this);
