...
var Client = (module.exports = function(config) {
  ...
  this.config.requestContentType =
    'application/vnd.kafka.v' + this.config.version + '+json';

  if (config.headers) this.config.headers = config.headers;

  this.config.timeout =
    config.timeout === undefined ? this.DEFAULTS.timeout : config.timeout;
...
Client.prototype._handleRequestResponse = function(cb, res) {
  ...
  res.on('end', function() {
    ...
    if (res.statusCode >= 400) cb(new APIError(res.statusCode, parsed));
    else cb(null, parsed);
  });
};
